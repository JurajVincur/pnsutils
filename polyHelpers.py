...
def calcCoeffs(data, cameraMatrix, distCoeffs, rot, fisheye=True, polynomialDegree=3):
  ...
  undirstortFunc = cv2.fisheye.undistortPoints if fisheye is True else cv2.undistortPoints
  non_zero_coordinates = undirstortFunc(non_zero_pixel_coordinates, cameraMatrix, distCoeffs, R=rot).reshape(-1, 2)
